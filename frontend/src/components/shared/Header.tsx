...
import { motion, AnimatePresence, useScroll, useTransform, animate, useMotionTemplate } from "framer-motion";
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3 text-lg font-semibold text-[var(--text)]">
          ...
          <div className="hidden sm:block leading-tight">
            <div className="font-bold text-[var(--brand)]">Yarno Chedemail</div>
            <div className="text-xs text-[var(--muted)]">Full Stack Developer</div>
          </div>
        </Link>
        ...
        <nav aria-label="Primary" className="relative flex items-center gap-3">
          ...
          <Link
            to="/login"
            className="hidden sm:inline-flex items-center gap-2 px-3 py-1.5 rounded text-sm border border-[var(--border)] hover:bg-[var(--border)]/30 transition cursor-pointer"
          >
            Admin
          </Link>
